...
"""
scripts/sync.py
Collecte quotidienne Spotify → Turso Cloud.
Exécuté par GitHub Actions via sync.yml.
"""
...
async def sync() -> None:
    # ── Lecture des secrets DANS main() — pas au niveau module ──────────────
    ...
    TURSO_URL             = require("TURSO_DATABASE_URL")
    TURSO_TOKEN           = require("TURSO_AUTH_TOKEN")
    ARTIST_SPOTIFY_ID     = require("ARTIST_SPOTIFY_ID")
    ...
    # Convertir libsql:// → https:// pour libsql-client
    url = TURSO_URL
    if url.startswith("libsql://"):
        url = url.replace("libsql://", "https://", 1)

    try:
        async with libsql_client.create_client(url=url, auth_token=TURSO_TOKEN) as client:
            ...
            if already and not FORCE_UPSERT:
                log.info("Déjà synchronisé aujourd'hui. Passe FORCE_UPSERT=true pour écraser.")
                return
            ...
            log.info("Turso OK [%s] — %s followers=%d pop=%d",
                     action, TODAY, followers, popularity)

    except Exception as exc:
        log.error("Échec Turso : %s", exc)
        sys.exit(1)
